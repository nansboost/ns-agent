// @ts-nocheck
import { sticker } from '../lib/sticker.ts'
import uploadFile from '../lib/uploadFile.ts'
import uploadImage from '../lib/uploadImage.ts'
import { webp2png } from '../lib/webp2mp4.ts'
...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try { 	
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      if (!img) throw `✳️ Balas gambar atau video dengan*${usedPrefix + command}*`
      let out
      try {
    ...
    else throw `Konversi gagal, coba kirim *gambar/video/gif* dulu lalu balas dengan perintah`
  }
}
