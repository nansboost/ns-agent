...
function sticker2(img, url) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let bufs = []
      const runtimeSupport = global.support || support
      const [_spawnprocess, ..._spawnargs] = [...(runtimeSupport.gm ? ['gm'] : runtimeSupport.magick ? ['magick'] : []), 'convert', 'png:-', 'webp:-']
...
function sticker6(img, url) {
  return new Promise(async (resolve, reject) => {
    ...
    // https://github.com/MhankBarBar/termux-wabot/blob/main/index.ts#L313#L368
 ...
 * Taken from https://github.com/pedroslopez/whatsapp-web.ts/pull/527/files
...
async function sticker(img, url, ...args) {
  ...
  for (let func of [
    ...
    global.support?.ffmpeg && (global.support?.convert || global.support?.magick || global.support?.gm || global.support?.imageMagick) && sticker2,
    sticker1
  ].filter(f => f)) {
    try {
