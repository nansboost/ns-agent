...
import { toAudio } from './converter.ts'
...
getJid: {
    async value(sender, chatId = null) {
        return resolveJid(conn, sender, { chatId })
    },
    enumerable: true
},
...
getLid: {
    value(sender) {
        ...
        if (isPnJid(sender)) return getCachedLidFromPn(sender) || sender
        return sender
    },
    enumerable: true
},
...
rememberJidPair: {
    value(lid, pn, source = 'manual') {
        return rememberJidPair(lid, pn, source)
    },
    enumerable: true
},
...
getNum: {
  async value(jid, chatId = null) {
    if (!jid) return null
    const resolved = await resolveJid(this, jid, { chatId })
    const pn = isPnJid(resolved) ? resolved : getCachedPnFromLid(resolved) || resolved
    if (!pn) return null
    return String(pn).split('@')[0].split(':')[0].replace(/[^0-9]/g, '') || null
  },
  enumerable: true
},
...
getName: {
  async value(jid = '', withoutContact = false) {
    try {
      ...
      // Kunci utama: ambil nama dari cache chat jika tersedia
...
export function serialize() {
    ...
    enumerable: true
       ...
       name: {
    get() {
        return this.pushName || this.sender?.split('@')[0] || 'Pengguna'
    },
    enumerable: true
},
        download: {
